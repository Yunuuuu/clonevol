...
make.clonal.data.frame <- function(vafs, labels, add.normal = FALSE,
    ...
    if (is.null(colors)) {
        ...
        if (v$lab[1] == "0") {
            colors <- c(normal.clone.color, colors)
        }
    }
    ...
    if (add.normal) {
        v <- rbind(data.frame(
            vaf = 0.5, lab = "0", color = colors[length(colors)],
...
enumerate.clones <- function(v, sample = NULL, variants = NULL,
    ...
    findParent <- function(v, i) {
        ...
        if (i > nrow(v)) {
            ...
            # print(head(v))
            vaf <- v[i, ]$vaf
            if (!is.na(v[i, ]$parent) && v[i, ]$parent == "-1") {
                # root
                vx <- v
                ...
                for (j in potential.pars.idx) {
                    ...
                    if (i != j && !v$excluded[j] && !is.ancestor) {
                        ...
                        sub.clusters <- as.character(c(
                            v$lab[!is.na(v$parent) &
    ...
    for (i in 1:nrow(v)) {
        ...
        if (is.null(min.cluster.vaf)) {
            ...
            t <- subclonal.test(sample,
                parent.cluster = cl, sub.clusters = NULL,
...
draw.clone <- function(x, y, wid = 1, len = 1, col = "gray",
    ...
    if (clone.shape == "polygon") {
        ...
        if (FALSE) {
            ...
            plot(cells$x, cells$y,
                pch = 20, axes = FALSE, col = "black", cex = 0.1,
    ...
    if (!is.na(cell.frac)) {
        ...
        if (cell.frac.side.arrow && cell.frac.position == "top.out") {
            ...
            arrows(
                x0 = x2, y0 = y2, x1 = x3, y1 = y3, col = cell.frac.side.arrow.col,
...
get.cell.frac.ci <- function(vi, include.p.value = TRUE, sep = " - ") {
    ...
    if ("free.lower" %in% colnames(vi)) {
        ...
        is.subclone <- determine.subclone(
            vi,
    ...
    draw.sample.clone <- function(i) {
        ...
        if (vi$vaf > 0) {
            ...
            if (!is.na(vi$parent) && vi$parent == -1) {
                ...
                if (vi$vaf < 0.05 && par$num.subclones > 1) {
                    x.shift <- x.shift * 2
                }
            ...
            if (!is.null(zero.cell.frac.clone.border.color) & vi$is.zero) {
                ...
                if (border.color == "fill") {
                    border.color <- clone.color
                }
            }
            ...
            draw.clone(xi, yi,
                wid = wid * vi$vaf, len = leni, col = clone.color,
...
plot.tree <- function(v, node.shape = "circle", display = "tree",
    ...
    if (!is.null(node.label.split.character)) {
        ...
        if (!is.null(node.num.samples.per.line)) {
            for (i in 1:length(vertex.labels)) {
                ...
                sel <- seq(
                    min(node.num.samples.per.line, length(vl)),
        ...
        vertex.labels <- paste0(extra.lf, gsub(
            node.label.split.character, "\n",
            vertex.labels
        ))
    }

    plot(g,
        edge.color = "black", layout = layout, main = title,
    ...
    if ((color.node.by.sample.group || color.border.by.sample.group) & show.legend &
        ...
        vi <- unique(v[
            !v$excluded & !is.na(v$parent),
            c("sample.group", "sample.group.color")
        ])
        vi <- vi[order(vi$sample.group), ]
        if (color.border.by.sample.group) {
            legend("topright",
                legend = vi$sample.group, pt.cex = 3, cex = 1.5,
        ...
        if ("events" %in% colnames(v)) {
            ...
            legend("topleft",
                legend = paste0(sprintf("%2s", ve$lab), ": ", ve$events),
    ...
    for (s in samples) {
        ...
        for (cl in labs) {
            ...
            if (length(sc) > 0) {
                ...
                r <- data.frame(
                    lab = cl, sample = s, subclones = sc, p = p,
                    stringsAsFactors = FALSE
                )
                if (is.null(subs)) {
                    subs <- r
...
merge.clone.trees <- function(trees, samples = NULL, sample.groups = NULL,
    ...
    for (i in 1:n) {
        ...
        if (nrow(v) == 0) {
            stop("ERROR: Something wrong. No clones left after filter. They might have been excluded.\n")
        }
        ...
        if (is.null(ccf.ci)) {
            ...
            ccf.ci <- rbind(ccf.ci, ci)
        }
        if (is.null(ccf.ci.nonzero)) {
            ccf.ci.nonzero <- ci.nonzero
        } else {
        ...
        this.lf <- data.frame(
            lab = this.leaves, leaf.of.sample = s,
        ...
        if (!is.null(sample.groups)) {
            # this is uneccesary if given default grouping above
            cg <- data.frame(
                lab = vz$lab, sample.group = sample.groups[s],
                stringsAsFactors = FALSE, row.names = NULL
            )
            if (is.null(cgrp)) {
                cgrp <- cg
...
trim.clone.trees <- function(merged.trees, remove.sample.specific.clones = TRUE,
    ...
    while (i < n) {
        j <- i + 1
        if (i > 1) {
            merged.trace <- rbind(merged.trace, c(idx[i], idx[i]))
        }
    ...
    # today debug
    # mtr <<- merged.trace
    if (is.null(dim(merged.trace))) {
        # one row, 1 tree to merge,
...
find.matched.models <- function(vv, samples, sample.groups = NULL,
        ...
        if (met.idx > nSamples) {
            ...
            for (j in 1:length(matched.models)) {
                ...
                if (matched.models[j] > 0) {
                    if (!match.sample.clones(
                        vv[[j]][[matched.models[j]]],
            ...
            if (match.with.all.models) {
                ...
                find.next.match(
                    prim.idx, prim.model.idx, met.idx + 1, 1,
                    matched.models, model.scores
                )
            } # else{
    ...
    for (prim.model in 1:length(vv[[1]])) {
        ...
        model.scores <- c(
            get.model.score(vv[[1]][[prim.model]]),
            rep(0, nSamples - 1)
        )
    ...
    return(list(
        models = vv, matched.models = matched, merged.trees = merged.trees,
...
infer.clonal.models <- function(c = NULL, variants = NULL,
    ...
    if (!is.null(p.value.cutoff)) {
        sum.p.cutoff <- p.value.cutoff
    }
    if (is.null(alpha)) {
        alpha <- sum.p.cutoff
    }
    ...
    num_vaf_col <- length(vaf.col.names)
    if (identical(num_vaf_col, 1L)) {
        added_vaf_col <- paste0(vaf.col.names, ".add_vaf_col")
        variants[[added_vaf_col]] <- variants[[vaf.col.names]]
        vaf.col.names <- c(vaf.col.names, added_vaf_col)
    }
    ...
    if (!is.null(cancer.initiation.model)) {
        model <- cancer.initiation.model
    }
    ...
    for (i in 1:nSamples) {
        ...
        if (length(models) == 0) {
            print(v)
            message(paste(
                "ERROR: No clonal models for sample:", s,
    ...
    results$boot <- boot
    if (identical(num_vaf_col, 1L)) {
        if (!is.null(results$variants)) {
            results$variants[[added_vaf_col]] <- NULL
        }
        results$models[[added_vaf_col]] <- NULL
        results$matched$index[[added_vaf_col]] <- NULL
        results$matched$scores[[added_vaf_col]] <- NULL
...
plot.clonal.models <- function(y, out.dir,
    ...
    if (!file.exists(out.dir)) {
        ...
        if (!overwrite.output) {
            stop(paste(
                "ERROR: Output directory (", out.dir,
                ") exists. Quit!\n"
            ))
        }
    }
    ...
    if (!is.null(matched$index)) {
        ...
        if (out.format == "pdf") {
            pdf(paste0(out.dir, "/", out.prefix, ".pdf"),
                width = w, height = h,
        ...
        for (i in 1:nrow(matched)) {
            ...
            if (out.format == "png") {
                png(paste0(this.out.prefix, ".png"),
                    width = w,
                    height = h, res = resolution, units = "in"
                )
            } else if (out.format == "pdf.multi.files") {
                pdf(paste0(this.out.prefix, ".pdf"),
                    width = w, height = h,
                    useDingbat = FALSE, title = ""
                )
            } else if (out.format != "pdf") {
                stop(paste0(
                    "ERROR: output format (", out.format,
                    ") not supported.\n"
                ))
            }
            ...
            if (merged.tree.plot) {
                mat <- cbind(mat, rep(nSamples * num.plot.cols + 1, nrow(mat)))
            }
            if (merged.tree.clone.as.branch) {
                mat <- cbind(mat, rep(nSamples * num.plot.cols + merged.tree.plot + 1, nrow(mat)))
            }
            # print(mat)
            if (is.null(panel.widths)) {
                ...
                if (length(panel.widths) != (num.plot.cols + merged.tree.plot + merged.tree.clone.as.branch)) {
                    stop(paste0(
                        "ERROR: panel.widths length does not equal # of plots ",
            ...
            for (k in 1:length(samples)) {
                ...
                if (disable.cell.frac) {
                    bell.plot.center.to.top <- 40
                }
                start.pos <- 1
                if (disable.sample.label) {
                    start.pos <- 0.1
                    lab <- ""
                }
                draw.sample.clones(m,
                    x = start.pos + sample.pos$xstarts[k], y = 0,
        ...
        # plot trimmed trees
        if (nrow(trimmed.trees[[1]]) == 0) {
            # single sample, no trimmed merged tree
        ...
        if (!is.null(merged.traces[[1]])) {
            ...
            for (i in 1:nrow(matched)) {
                ...
                if (is.null(all.traces)) {
                    ...
                    all.traces <- rbind(all.traces, tmp)
                }
            }
            ...
            write.table(all.traces, paste0(
                out.dir, "/", out.prefix,
                ".sample-reduction.tsv"
            ),
            sep = "\t", row.names = FALSE,
            quote = FALSE
            )
        ...
        # of !is.null(matched$index); plot all
    ...
    for (cl in clusters) {
        ...
        if (method == "median") {
            if (is.one.sample) {
                ...
                median.vafs <- apply(
                    v[v[[cluster.col.name]] == cl, vaf.col.names],
                    2, median
                )
            }
        } else if (method == "mean") {
    ...
    for (vaf.name in vaf.names) {
        ...
        for (i in base.clusters.idx) {
            ...
            for (j in (i + 1):nrow(clone.vafs)) {
                ...
                if (!clone.vaf.diff(
                    var[var[[cluster.col.name]] == ci, vaf.name],
...
rematchClonalPresence <- function(x1, x2, samples, merged.sample, adjust = TRUE) {
    ...
    for (i in 1:n) {
        ...
        m1found <- sapply(
            m1$sample.with.nonzero.cell.frac.noci,
            function(v) {
                any(samples %in% unlist(
        ...
        m2found <- sapply(
            m2$sample.with.nonzero.cell.frac.noci,
            function(v) {
                merged.sample %in% unlist(
                    strsplit(gsub("*", "", v, fixed = TRUE), ",")
                )
            }
        )
...
merge.all.matched.clone.trees <- function(x) {
    ...
    for (i in 1:x$num.matched.models) {
        ...
        zz <- merge.clone.trees(m,
            samples = samples, x$params$sample.groups,
...
assign.events.to.clones.of.a.tree <- function(tree, events, samples, cutoff = 0) {
    ...
    for (i in 1:nrow(events)) {
        ...
        tree$events[best.match.idx] <- paste0(
            tree$events[best.match.idx],
            events$event[i], ","
        )
    }
...
plot.cell.population <- function(cell.frac, colors, label = "", label.size = 1,
    ...
    if (layout == "cloud") {
        ...
        plot(x, y,
            col = cell.border.color, bg = cols, lwd = cell.border.size,
            axes = FALSE, cex = cell.cex,
            pch = 21, xlim = c(1 - delta, n + delta), ylim = c(1 - delta, n + delta)
        )
    } else {
        stop(paste0(
            "ERROR: plot cell population layout=", layout,
            " not supported.\n"
        ))
    }
...
plot.cloud.of.cells <- function(cells, title = "", title.size = 1,
    ...
    p <- (ggplot(cells) +
        geom_polygon(aes(x, y, group = id, fill = color),
